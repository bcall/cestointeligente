...
<title>Proyecto - Puntos Verdes</title>
<c:set var="sectionLabel" value="Detalle Proyecto" scope="session" />
...
<body>
	...
	<div class="container">
		<section style="height: 70vh;">
								...
								<td colspan="2"><div id="pStat" class="alert"
										style="margin: 0; padding: 5px; text-align: center;" hidden="true">${proyect.STATUS}</div></td>
...
<div id="overlay"
	...
	<script src="../js/sections/proy-details.js"></script>
	...
	function loadData(val){
		...
		$.get("../services/islaTableData?idpro="+${proyect.ID}, function(data) {
			...
			dataTable = $('#topTable').dataTable({
			 "columnDefs": [ {
                ...
                "render": function ( data, type, full, meta ) {
                          return '<a href="../sections/isladetails?'+data+'"class="fa fa-arrow-circle-right"></a>';
                                  }
                } ],
...
$(function() {
	...
	$.get("../services/indexGraph?proyecto_id=${proyect.ID}", function(data) {
		graphNotReci(data);
		graphReci(data);
	});
});
...
function graphReci(wsData) {
	...
	var data = {
		...
		datasets : [ {
			...
			fillColor : "rgba(155, 139, 235,0.2)",
			strokeColor : "#9b8beb",
			pointColor : "#9b8beb",
...
function graphNotReci(wsData) {
	...
	var data = {
		...
		datasets : [ {
			label : "Peso Proyectado",
			fillColor : "rgba(102, 102, 153,0.15)",
			strokeColor : "#5a5a8c",
			pointColor : "#5a5a8c",
			pointStrokeColor : "#fff",
			pointHighlightFill : "#fff",
