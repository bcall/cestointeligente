...
$(function() {
	...
	$.get("../cestointeligente/services/indexGraph", function(data) {
		graphDiaria(data);
		graphAumulado(data);
...
function graphDiaria(wsData) {
	...
	var dataDaysRec = [];
	var dataDaysNoRec = [];
	var daysInMontRec = new Date(daysRec[0].DATE.split('-')[0], daysRec[0].DATE
			.split('-')[1], 0).getDate();
	0
	var daysInMontNoRec = new Date(daysNoRec[3].DATE.split('-')[0],
			daysNoRec[0].DATE.split('-')[1], 0).getDate();
	var totalBday = total / daysInMontRec;
	for (var i = 0; i < daysInMontRec; i++) {
		labels[i] = i + 1;
		dataDaysRec[i] = 0;
		dataDaysNoRec[i] = 0;
	}
...
function graphAumulado(wsData) {
	...
	for ( var i in daysNoRec) {
		 dataDaysNoRec[i] = totalW = (parseInt(totalW) +
